...
import { ChevronLeft, Users, Clock, Save, ArrowLeft, CheckCircle2, AlertCircle } from 'lucide-react-native';
...
export default function Suivi() {
  ...
  const fetchApprenantsAndPresences = async (seance: Seance) => {
    ...
    apprenantsList.forEach(app => {
      ...
      pMap[app.id] = ex ? { status: ex.status, heures_validees: ex.heures_validees, comment: ex.comment || '', justificatif_id: ex.justificatif_id } : { status: 'present', heures_validees: duree, comment: '' };
    });
  ...
  const handleStatusChange = (id: string, st: 'present' | 'absent' | 'retard' | 'excuse') => {
    setPresencesData(p => {
      ...
      return { ...p, [id]: { ...c, status: st, heures_validees: (st === 'absent' || st === 'excuse') ? 0 : (c.heures_validees === 0 && st === 'present' ? d : c.heures_validees) } };
    });
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = Object.entries(presencesData).map(([appId, data]) => ({ seance_id: selectedSeance.id, apprenant_id: appId, status: data.status, heures_validees: data.heures_validees, comment: data.comment }));
    ...
    else { Alert.alert("Succès", "Appel enregistré !"); setSelectedSeance(null); }
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-gray-50">
        ...
        {loading ? <ActivityIndicator color="#006eb8" /> : apprenants.map(app => {
          ...
          return (
            <View key={app.id} className="bg-white p-4 rounded-[24px] mb-4 border border-gray-100 shadow-sm">
              ...
              <View className="flex-row justify-between bg-gray-50 p-1 rounded-2xl">
                {['present', 'absent', 'retard', 'excuse'].map((st) => (
                  <TouchableOpacity key={st} onPress={() => handleStatusChange(app.id, st as any)} className={`flex-1 py-2 mx-0.5 rounded-xl items-center ${state.status === st ? (st==='present'?'bg-green-500':st==='absent'?'bg-red-500':st==='retard'?'bg-orange-400':'bg-blue-500') : 'bg-transparent'}`}>
      ...
      <View className="absolute bottom-6 left-6 right-6">
        <TouchableOpacity onPress={handleSave} disabled={saving} className="bg-brand-600 py-4 rounded-2xl flex-row justify-center items-center shadow-2xl shadow-brand-400">
          {saving ? <ActivityIndicator color="#fff" /> : <><Save size={20} color="#fff" /><Text className="text-white font-black uppercase tracking-widest ml-2">Enregistrer</Text></>}
        </TouchableOpacity>
      </View>
